			...
			for (; il < exploded.length; il++) {
				...
				try {
					...
					if (ok) {
						...
						if (Pattern.compile("^" + _att + msk_name_coord1,
								Pattern.CASE_INSENSITIVE).matcher(line).find()) {
			...
			try {
				...
				if (ok || RC_end) {
					...
					if (Pattern.compile("^" + _att + msk_name_coord1, Pattern.CASE_INSENSITIVE)
						...
						if (!RC_end)
							textchanged += enter
									+ tmp[0].replaceAll(_att + msk_name_coord2, repl) + enter;
						if (att_coord == null) {
						...
						if (!RC_end)
							textchanged += enter
									+ tmp[0].replaceAll(_def + msk_name_coord2, repl) + enter;
						if (def_coord == null) {
				...
				if (ok) {
					if (Pattern.compile(msk_emp_coord, Pattern.CASE_INSENSITIVE)
						...
						/* LUDO */
							...
							&& Pattern.compile(msk_emp_goods1, Pattern.CASE_INSENSITIVE)
									.matcher(line)
									.find()) {
						type++;
						if (type >= 0 && type <= 2) {
							rssName[type] = line.replaceAll(msk_emp_goods1, "$1");
							Matcher ressMatcher = Pattern.compile(msk_emp_goods2)
									.matcher(line);
							int index = 0;
